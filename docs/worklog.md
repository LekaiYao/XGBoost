...
- 历史输入存在混配风险：曾出现 `23MC + 24DATA` 的 fallback 逻辑，不符合“按年份成组一致”要求。
- POST_BATCH 参数错位风险：空参数在 submit/脚本位置参数链中塌缩，可能把 `auto` 误当作 DATA 输入路径。
- 指令与运行不一致风险：DAG 写入 `fid3`/`dataset_year=2024` 后，实际运行仍可能落到 `FID profile: auto` 与空 dataset_year。
- AFS/EOS 同步风险：提交侧与代码侧文件不同步会导致“代码已改、运行仍旧配置”的问题反复出现。
